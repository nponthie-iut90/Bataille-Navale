...
public class StageModelFactory extends StageElementsFactory {
    ...
    public void setup() {
        ...
        // attack marks for player 1 board
        for (int i = 0; i < 99; i++) {
            AttackMark attackMark = new AttackMark(stageModel, 0);
            stageModel.getPlayer1AttackMarksForOwnBoard().add(attackMark);
            stageModel.addElement(attackMark);
        }
        // attack marks for player 2 board
        for (int i = 0; i < 99; i++) {
            AttackMark attackMark = new AttackMark(stageModel, 0);
            stageModel.getPlayer2AttackMarksForOwnBoard().add(attackMark);
            stageModel.addElement(attackMark);
        }


        TextElement log = new TextElement(stageModel.getLogText(), stageModel);
        stageModel.setLog(log);
        log.setLocation(10, 600);
    }
}
